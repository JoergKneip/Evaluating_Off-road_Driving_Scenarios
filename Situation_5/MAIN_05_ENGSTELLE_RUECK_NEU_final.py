...
STL_2_FILENAME = r"C:\Users\micha\OneDrive\Desktop\Drohnenvideos\Python und Drohne\ENGSTELLE.stl"
...
MODEL_2_SETTINGS = {
    'scale': 0.001, 'auto_center': True, 'offset': [0, 0, 0], 
    'rotation': [0, 0, 0], 
...
def get_tilted_model_traces(filename, settings, tilt_matrix, name_label):
    ...
    try:
        ...
        print(f"Error beim Laden von {filename}: {e}")
        return []
    return traces
...
def add_detailed_marker_with_axes(fig, tilt_matrix):
    ...
    for mid, cfg in MARKER_CONFIG.items():
        ...
        for vec_local, color, name in axes_def:
            ...
            fig.add_trace(go.Scatter3d(x=[center_w[0], end[0]], y=[center_w[1]], z=[center_w[2]], mode='lines', line=dict(color=color, width=5), name=f'M{mid} {name}', showlegend=False))
...
def run_visualization(tilt_matrix):
    ...
    # --- DICHTE BERECHNUNG & GELBE FLÄCHE (FRONT / X < 0) ---
    if len(all_centers) >= 1:
        points_array = np.array(all_centers)
        views_array = np.array(all_views)

        # 1. FILTER: NUR PUNKTE HINTER DEM FAHRZEUG (X < 0)
        # (Front Marker sind bei 2.3m, also ist alles > 0 die Heckseite)
        mask_front = points_array[:, 0] > 0
        
        cluster_points = points_array[mask_front]
        cluster_views = views_array[mask_front]
        
        num_valid = len(cluster_points)

        if num_valid > 0:
            print(f"-> Berechne Durchschnitt aus {num_valid} Videos (Bereich: Vorne / X < 0).")

            # 2. FILTER: AUSREISSER ENTFERNEN (Abstand zum Median > 1m)
            median_center = np.median(cluster_points, axis=0)
            distances_to_center = np.linalg.norm(cluster_points - median_center, axis=1)
            
            OUTLIER_DIST_LIMIT = 4.0
            mask_clean = distances_to_center <= OUTLIER_DIST_LIMIT
            
            points_clean = cluster_points[mask_clean]
            
            # Statistik
            removed_count = len(cluster_points) - len(points_clean)
            if removed_count > 0:
                print(f"   -> {removed_count} Ausreißer entfernt (> {OUTLIER_DIST_LIMIT}m vom Zentrum).")

            # Fallback falls alles gelöscht wurde
            if len(points_clean) < 4: points_clean = cluster_points

            # Durchschnitt berechnen
            best_pos = np.mean(points_clean, axis=0)
            
            # Views filtern
            views_clean = cluster_views[mask_clean]
            if len(views_clean) > 0: best_view = np.mean(views_clean, axis=0)
            else: best_view = np.mean(cluster_views, axis=0)

            if np.linalg.norm(best_view) > 0:
                best_view = best_view / np.linalg.norm(best_view)

            # --- GOLDENE FLÄCHE (Convex Hull der bereinigten Punkte) ---
            fig.add_trace(go.Mesh3d(
                x=points_clean[:,0], y=points_clean[:,1], z=points_clean[:,2],
                alphahull=0,        # 0 = Exakte Konvexe Hülle (Kantig)
                color='gold', 
                opacity=0.3,        # Transparent
                name='Relevante Fläche (Front)',
                showlegend=True
            ))
            ...
            fig.add_trace(go.Scatter3d(
                ...
                text=["DURCHSCHNITT (FRONT)"], textposition="top center",
                textfont=dict(size=14, color="black"), name='Schnittmenge (Front)'
            ))
            ...
            print("-> WARNUNG: Keine Datenpunkte im Frontbereich (X < 0) gefunden!")
    ...
    fig.update_layout(
        title=f"Analyse: Durchschnitt + Bereich (Front, gefiltert)",
